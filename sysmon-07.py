...
class RealtimeMonitor:
    def __init__(self, max_points=60, smooth_window=5):
        self.max_points = max_points  # Show last 60 data points
        ...
        self.metrics_data = {
            ...
            'Network_Upload': deque(maxlen=max_points),
            'Network_Download': deque(maxlen=max_points)
        }
        ...
        try:
            if os.path.exists(self.config_file):
                ...
                if 'window_size' in config:
                    ...
                    manager.window.setGeometry(config.get('x', 100), config.get('y', 100), width, height)
    ...
    def save_window_geometry(self):
        ...
        try:
            manager = self.fig.canvas.manager
            geometry = manager.window.geometry()
    ...
    def animate(self, frame):
        ...
        if len(time_array) > 0:
            ...
            # Auto-scale y-axis based on both read and write (using smoothed data)
            all_disk_smooth = list(disk_read_smooth) + list(disk_write_smooth)
            if len(all_disk_smooth) > 0:
                max_val = max(all_disk_smooth) if max(all_disk_smooth) > 0 else 10
        ...
        if len(time_array) > 0:
            ...
            # Auto-scale y-axis based on both upload and download (using smoothed data)
            all_network_smooth = list(upload_smooth) + list(download_smooth)
            if len(all_network_smooth) > 0:
                max_val = max(all_network_smooth) if max(all_network_smooth) > 0 else 10
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='''Examples:
  python sysmon.py                    # Run with default settings
  python sysmon.py -s 10              # Run with heavy smoothing
  python sysmon.py --smooth-window 3  # Run with light smoothing
''')
    ...
    monitor = RealtimeMonitor(max_points=args.max_points, smooth_window=args.smooth_window)
    monitor.start()
